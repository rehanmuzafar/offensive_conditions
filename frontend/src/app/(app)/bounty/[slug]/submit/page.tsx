...
export default function SubmitReportPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const canSubmit = title.trim().length >= 10 && asset.trim() && description.trim().length >= 30 && steps.trim().length >= 20 && impact.trim().length >= 10;
  ...
  function onSubmit() {
    ...
    submit.mutate(
      { title, assetIdentifier: asset, severity, descriptionMd: description, reproductionSteps: steps, impact, cvssVector: cvss || undefined },
      {
        ...
        onError: () => router.push("/bounty/reports"),
      },
    );
  }
        ...
        <CardBody className="space-y-1">
            ...
            <div className="flex flex-wrap gap-2">
              {SEVERITIES.map((s) => (
                <button
                  ...
                  className={cn("rounded-full border px-3 py-1.5 transition-colors", severity === s ? "border-accent bg-brand-gradient-soft" : "border-line-strong hover:bg-surface-hover")}
                >
          ...
          <div className="mb-4">
            ...
            {preview ? (
              ...
              <textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={6} placeholder="Describe the vulnerability. Markdown supported." className="w-full resize-y rounded-xl border border-line-strong bg-bg-elevated px-3.5 py-3 text-[14.5px] text-text placeholder:text-text-faint focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/30" />
            )}
          </div>
