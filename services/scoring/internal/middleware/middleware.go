...
// RequireAuth attaches the JWT claims to the request context.
...
// OptionalAuth tries to validate the token if present, but doesn't fail if absent.
// Used for public leaderboards where authenticated users get extra fields (rank).
func OptionalAuth(validator *auth.Validator) gin.HandlerFunc {
	return func(c *gin.Context) {
		token := extractToken(c)
		if token == "" {
			c.Next()
