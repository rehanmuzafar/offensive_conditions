...
from app.grpc.server import ContentGRPCServer
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    log.info("starting_content_svc", env=settings.app_env, version=settings.app_version)
    ...
    # Kafka publisher
    publisher = ContentEventPublisher(settings)
    try:
    ...
    # gRPC server
    grpc_server = ContentGRPCServer(settings)
    ...
    try:
        ...
        log.info("shutting_down_content_svc")
...
def create_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(
        title="Offensive Conditions — Content Service",
...
class RequestContextMiddleware(BaseHTTPMiddleware):
    """Inject a request_id, log timings, and propagate the header."""
