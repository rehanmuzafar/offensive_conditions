 ...
 *
 * Starts: Fastify HTTP API on :8007, gRPC on :9007, Kafka producer, Redis
 * connection. Graceful shutdown drains all in flight connections.
 */
...
import { registerPlanRoutes } from '@/api/v1/plans.js';
...
async function main(): Promise<void> {
  ...
  log.info({ env: cfg.APP_ENV, version: cfg.APP_VERSION }, 'starting_payment_svc');
  ...
  const app = Fastify({
    logger: false,  // we install our own via middleware
