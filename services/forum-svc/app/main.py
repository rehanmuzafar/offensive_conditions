...
from app.grpc.server import ForumGRPCServer
from app.services import ForumEventPublisher
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    log.info("starting_forum_svc", env=settings.app_env, version=settings.app_version)
    ...
    publisher = ForumEventPublisher(settings)
    try:
    ...
    grpc_server = ForumGRPCServer(settings)
    ...
    try:
        ...
        log.info("shutting_down_forum_svc")
...
def create_app() -> FastAPI:
    settings = get_settings()
    app = FastAPI(
        title="Offensive Conditions — Forum Service",
