-- =============================================================================
-- Notification Schema — Initial Migration
-- =============================================================================
-- Generated from services/notification-svc/src/migrations/
-- 20260526_0001_notification_schema.ts, which was never wired into any runner,
-- so the notification schema did not exist at all and every /v1/me/notifications
-- request 500'd on `relation "notification.notifications" does not exist`.
-- =============================================================================
...
CREATE INDEX idx_templates_event ON notification.templates (event_type, channel, locale) WHERE is_active = TRUE;
...
CREATE INDEX idx_notifications_user_unread ON notification.notifications (user_id, created_at DESC) WHERE read_at IS NULL AND deleted_at IS NULL;

CREATE INDEX idx_notifications_user_created ON notification.notifications (user_id, created_at DESC) WHERE deleted_at IS NULL;

CREATE INDEX idx_notifications_event ON notification.notifications (event_id);
...
CREATE INDEX idx_webhooks_user ON notification.webhooks (user_id) WHERE is_active = TRUE;
...
CREATE INDEX idx_deliveries_user_created ON notification.deliveries (user_id, created_at DESC);

CREATE INDEX idx_deliveries_event ON notification.deliveries (event_id, channel);

CREATE INDEX idx_deliveries_status ON notification.deliveries (status, created_at DESC) WHERE status IN ('pending','failed');
...
CREATE INDEX idx_consumed_topic_offset ON notification.consumed_events (source_topic, kafka_offset DESC);
